...
class phone {
    ...
    public static function add_phone_to_form(
        ...
        $group = [
            $mform->createElement('autocomplete', 'code', '', self::get_country_codes_options($fullstring), $options, ['size' => 10,]),
     ...
     * @return string[]
     */
    public static function validate_phone_from_submitted_data($data) {
        $data   = (array)$data;
        $errors = [];

        foreach ($data as $key => $value) {
            if (is_array($value) and isset($value['code']) && isset($value['number'])) {
                if (!self::validate_number($value['code'], $value['number'], true, false, true)) {
                    $errors[$key] = get_string('profileinvaliddata', 'admin');
                }
            }
        }
     ...
     * @link {https://github.com/AfterShip/phone/blob/master/src/data/country_phone_data.ts}
     * @return array[]
     */
